...
function FreshnessBadge({ model }: { model: ModelProfile }) {
  const meta = model.researchMeta;
  const daysSinceEval = Math.floor(
    (Date.now() - new Date(meta.lastEvaluatedDate).getTime()) / (1000 * 60 * 60 * 24)
  );
  const confidence = meta.sourceConfidence;

  let level: "high" | "medium" | "needs_review";
  let color: string;
  let borderColor: string;

  if (meta.needsReEvaluation || daysSinceEval > 90 || confidence < 0.5) {
    level = "needs_review";
    color = "text-amber-400 bg-amber-500/10";
    borderColor = "border-amber-500/20";
  } else if (daysSinceEval > 45 || confidence < 0.7) {
    level = "medium";
  ...
  const labelMap = { high: "High confidence", medium: "Medium confidence", needs_review: "Needs review" };
  const iconMap = {
    high: <CheckCircle className="h-3.5 w-3.5" />,
    medium: <Clock className="h-3.5 w-3.5" />,
    needs_review: <RefreshCw className="h-3.5 w-3.5" />,
  };

  return (
    <div className={`inline-flex items-center gap-3 rounded-xl border ${borderColor} ${color} px-4 py-2.5 text-xs font-medium`}>
      ...
      <span className="text-zinc-400">
        Updated {daysSinceEval === 0 ? "today" : daysSinceEval === 1 ? "yesterday" : `${daysSinceEval}d ago`}
      </span>
      <span className="text-zinc-500">|</span>
      <span className="text-zinc-400">
...
function ModelQualityBreakdown({ modelId }: { modelId: string }) {
  const [data, setData] = React.useState<any>(null);
  const [show, setShow] = React.useState(false);

  React.useEffect(() => {
    fetch(`${API}/panel/quality/model/${modelId}`)
      .then((r) => r.ok ? r.json() : null)
      .then((d) => { if (d && d.times_recommended > 0) { setData(d); setShow(true); } })
      .catch(() => {});
  }, [modelId]);
  ...
  return (
    <div className="reveal-step" style={{ animationDelay: "0.15s" }}>
      <h2 className="mb-3 panel-label">Recommendation quality</h2>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="panel-card p-4">
          <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Recommended</p>
          <p className="text-2xl font-black text-white mt-1">{data.times_recommended}</p>
        </div>
        <div className="panel-card p-4">
          <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Acceptance</p>
          <p className="text-2xl font-black text-emerald-400 mt-1">{data.acceptance_rate != null ? `${(data.acceptance_rate * 100).toFixed(0)}%` : "—"}</p>
        </div>
        <div className="panel-card p-4">
          <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Override from</p>
          <p className="text-2xl font-black text-amber-400 mt-1">{data.override_from_rate != null ? `${(data.override_from_rate * 100).toFixed(0)}%` : "—"}</p>
        </div>
        <div className="panel-card p-4">
          <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Avg satisfaction</p>
          <p className="text-2xl font-black text-blue-400 mt-1">{data.avg_satisfaction?.toFixed(2) ?? "—"}</p>
        </div>
      </div>
      {data.top_override_reasons?.length > 0 && (
        <div className="mt-3 panel-card p-4">
          <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 mb-2">Top override reasons</p>
          <div className="flex flex-wrap gap-1.5">
            {data.top_override_reasons.map(([reason, count]: [string, number]) => (
              <span key={reason} className="rounded-full bg-white/[0.04] border border-white/[0.06] px-2.5 py-1 text-[10px] text-zinc-400">
...
export default function ModelDetailClient({ model }: { model: ModelProfile }) {
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      {/* Recommendation Quality (Sprint 3.4) */}
      <ModelQualityBreakdown modelId={model.id} />
